...
When looking at quality assurance in a data analysis it appears to be useful to differentiate between the following time periods during a data analysis. 

1. Planning phase. 
2. Execution of actual data analysis.
3. Post-analysis period. 

```{r}
this_wd <- getwd()
this_rmd <- list.files(this_wd, pattern = "Rmd$")

rmddochelper::use_odg_graphic(ps_odg_file = "../odg/PeriodDataAnalysis.odg")
```
