...
# conversion hook
knitr::opts_chunk$set(odg_conv = rmddochelper::odg_convert_hook)
```
...
When looking at quality assurance in a data analysis it appears to be useful to differentiate between different time periods that occur in a data analysis. These time periods are shown in the diagram below.

```{r da_period, odg_conv=TRUE, fig_path='../odg', out_format='png'}
rmddochelper::use_odg_graphic(ps_path = '../odg/da_period.odg')
```



The first time period corresponds to the period before the analysis is executed. 
