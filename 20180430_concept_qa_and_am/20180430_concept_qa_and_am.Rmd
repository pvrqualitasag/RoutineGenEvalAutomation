...
# Quality Assurance (QA) in Data Science (DS)
Data science has become a buzzword 

The really essential concept that allows for any measures of quality assurance (QA) is __reproducibility__. At the same time, it has to be emphasized that reproducibility alone does not get us anything in terms of QA, but it is the most important pre-requisite to be able to implement any measure of QA. Or the other way around, without reproduciblity there is no way that any statements about QA can be made after an analysis is completed.
...
The term _reproducibility crisis_ or replication crisis has its own [Wikipedia entry](https://en.wikipedia.org/wiki/Replication_crisis) 
